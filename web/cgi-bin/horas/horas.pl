...
sub Alleluia : ScriptFunc {
  ...
  our (%prayers, $incipitTone);
	...
	if ($lang =~ /gabc/i && $incipitTone) {
		$text = ($incipitTone =~ /festal/i) ? prayer('Alleluia1', $lang)
...
sub antiphona_finalis : ScriptFunc {
    ...
    || ($month == 2 && $day == 2 && $hora !~ /Completorium/i))
  {
    ...
    $name = ($lang !~ /gabc/i || $dayofweek) ? 'Paschalis' : 'Paschalis0';
  } else {
    $name = 'Postpentecost';
  }
...
sub Divinum_auxilium : ScriptFunc {
  ...
  $text[-2] = "V. $text[-2]";
  $text[-1] =~ s/.*\. // unless ($version =~ /Monastic/i && $version !~ /Bavariae/i); # contract resp. "Et cum fratribus… " to "Amen." for Roman
  $text[-1] = "R. $text[-1]";
  join("\n", @text);
}
