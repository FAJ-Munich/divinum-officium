...
sub preces {
  ...
  if (
    $dayofweek
    && !($dayofweek == 6 && $hora =~ /vespera/i)    # Domincales at Completorium are also said if Feriales at Vespers!
    && (
...
sub psalmi_minor {
  ...
  if ($hora !~ /completorium/i) {
    ...
    $ant = '' if $version =~ /Monastic/ && $version !~ /Bavariae/i;
  }
      ...
      foreach my $commemo (@centries) {
        ...
        if ($c) {
          ...
          if ($cr[0] =~ /Vigilia Epi|$sundaystring/i) {
            $key =
              ($version !~ /trident/i || ($version =~ /1906|Bavariae/i && $cr[2] > 5))
              ? 7000
...
sub getcommemoratio {
  ...
  if ($rank[3] =~ /(ex|vide)\s+(.*)\s*$/i) {
    ...
    if ($file =~ /^C[0-7]+[ab]?$/ && $dayname[0] =~ /Pasc/i) { $file .= 'p'; }
    $file = "$file.txt";
